#ifndef ESOM_CUDA_POINTS_HPP
#define ESOM_CUDA_POINTS_HPP
...
public:
	DataPoints(std::size_t dim = 0, std::size_t size = 0): mDim(dim), mSize(size), mDataPtr(nullptr)
	{
		resize(size);
	}
	...
	void resize(std::size_t size)
	{
		if (mFile.opened()) {
			...
			if (size > maxSize) {
				throw (bpp::RuntimeError() << "Unable to change size to" << size
					<< ". MMaped file holds only " << maxSize << " records.");
			}
			mSize = size;
		}
		else {
	...
	const F* operator[](std::size_t idx) const
	{
		return mDataPtr + (idx * mDim);
	}
	
	F* operator[](std::size_t idx)
	{
		return mDataPtr + (idx * mDim);
	}
	...
	void loadTSV(const std::string& fileName, std::size_t limit = std::numeric_limits<std::size_t>::max())
	{
		...
		while (mSize < limit && file.readLine(line)) {
			...
			tokenizer.doTokenize(line, tokens, true);
			if (tokens.size() < mDim) {
				throw (bpp::RuntimeError() << "Insufficient values on row #" << mSize
					<< ". Dimension " << mDim << " expected, but only " << tokens.size() << "tokens found.");
			}
	...
	void loadBinary(const std::string& fileName, std::size_t limit = std::numeric_limits<std::size_t>::max())
	{
		...
		if (fileSize % sizeof(F) != 0) {
			mFile.close();
			throw (bpp::RuntimeError() << "Size of file " << fileName << " is not divisible by float size (" << sizeof(F) << "). Invalid data expected.");
		}

		fileSize /= sizeof(F);
		if (mDim == 0 || fileSize % mDim != 0) {
			mFile.close();
			throw (bpp::RuntimeError() << "Size of file " << fileName << " is not divisible by feature dimensions (" << mDim << "). Invalid data expected.");
		}
